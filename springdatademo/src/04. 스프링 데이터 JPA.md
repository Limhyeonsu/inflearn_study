...
### 37. 스프링데이터 JPA: EntityGraph
쿼리 메서드마다 연관 관계의 Fetch 모드를 설정할 수 있다.

```java
//Post 객체를 기본은 LAZY로 하지만 필요한 경우에 EAGER로 가져오고 싶을때
...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    //comment 엔티티에서 설정한 post라는 연관관계를 EAGER 모드로 가져온다
    @EntityGraph(value = "Comment.post")
    Optional<Comment> getById(Long id);
}
```

* @NamedEntityGraph : @Entity에서 재사용할 여러 엔티티 그룹을 정의할 때 사용
* @EntityGraph : @NamedEntityGraph에 정의되어 있는 엔티티 그룹을 사용

### 38. 스프링데이터 JPA: Projection
Projection은 엔티티의 속성들이 너무 많을때 그중 일부분에 대한 데이터만 조회하고 싶은 경우 사용

* 인터페이스 기반
  * Closed 프로젝션
    * 쿼리 최적화 가능, 디폴트 메서드를 사용해서 연산 가능
  * Open 프로젝션 : 다 조회해온 다음에 그중에서 내가 필요한 데이터만 보는 것 또는 연산을 통해 조합 가능
    * @Value(SpEL)을 사용해서 연산 가능, 스프링 빈의 메서드도 호출 가능
    * 쿼리 최적화 X
...
public interface CommentSummary { 
    ...
    //default 메서드를 사용해서 한정된 필드에 대한 연산 가능
...
public interface CommentSummary {
  ...
  //Open Projection 방식 : 모든 데이터를 다 읽어온 다음에 연산
  @Value("#{target.up + ' ' + target.down")
  String getVotes();
}
```

똑같은 쿼리인데 리턴 타입만 다르게 하고 싶은경우 메서드 오버라이딩이 안되기 때문에 다음과 같이 한다.
```java
public interface CommentRepository extends JpaRepository<Comment, Long> {
...
* 클래스 기반 : 필드와 생성자, getter, setter가 필요해서 장황해진다.
  * Closed 프로젝션
  * Open 프로젝션

### 39. 스프링데이터 JPA: Specifications
QueryDSL의 Predicate와 비슷하다. (설정이 번거로움)
* 의존성 설정 : hebernate-jpamodelgen
* 플러그인설정
* IDE에 애너테이션 처리기 설정
* 코딩 시작

### 40. 스프링데이터 JPA: Query by Example
Example = Prove + ExampleMatcher
* Prove : 필드에 어떤 값들을 가지고 있는 도메인 객체
* ExampleMatcher : Prove에 들어있는 필드의 값들을 어떻게 쿼리 데이터와 비교할지 정의한 것

### 40. 스프링데이터 JPA: 트랜잭션
스프링 데이터 JPA가 제공하는 Repository의 모든 메서드에는 기본적으로 @Transactional이 적용되어 있다. 클래스, 인터페이스, 메서드에 사용할 수 있고, 메서드에 가장 가까운 애너테이션이 우선순위가 높다.

RuntimeException이 발생하면 rollback을 시키고 checked exception은 롤백시키지 않음

* noRollbackFor, noRollbackForClassName : 특정 Exception에는 rollback이 되지 않게 설정
* isolation : 트랜잭션 접근 설정
* propagation : 트랜잭션을 어떻게 전파할 것인지

readOnly를 설정하는 경우 Flush 모드를 NAVER로 설정하여 Dirty Checking이 발생하지 않는다. 데이터를 많이 가져오는 경우 더티 체킹을 막기만 해도 성능이 좋아진다.

### 41. 스프링데이터 JPA: Auditing
Auditing은 엔티티의 변화가 발생했을 때 그 변화가 언제, 누구에 의해 발생했는지를 기록하는 기능이다.

1. 스프링 부트에서도 자동으로 설정해주지 않으므로 메인 애플리케이션 위에 @EnableJpaAuditing 추가해야함.
2. 엔티티 클래스 위에 @EntityListeners(AuditingEntityListener.class) 리스너 등록 (날짜정보가 자동으로 등록됨)
3. 계정정보 ex) Account 같은 경우 AuditorAware 구현체를 만들어야 한다.
...
@Service //빈으로 등록하기 위함
//AuditorAware은 스프링 시큐리티에서 유저정보를 가져온다.
...
__@DataJpaTest는 슬라이싱 테스트라 데이트 관련된 계층에 대한 빈만 등록해준다. @Service가 붙은 빈은 찾지 못한다. 그래서 모든 빈에 대한 정보다 필요한 경우 @SpringBootTest를 사용한다.__

[JPA의 라이프사이클 이벤트]

JPA의 라이프사이클 이벤트는 엔티티의 변화가 일어났을 때 특정한 콜백을 실행할 수 있는 이벤트를 발생시켜준다.
* @PrePersist : 엔티티 저장전 호출
* @PreUpdate
